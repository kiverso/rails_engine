...
RSpec.describe 'Business inteligence endpoints', type: :request do
  before(:each) do
    ...
    @invoice1 = Invoice.create({ customer_id: @customer.id, merchant_id: @merchant1.id,
                                 created_at: '2012-03-23 09:54:09 UTC' })
    @invoice2 = Invoice.create({ customer_id: @customer.id, merchant_id: @merchant2.id,
                                 created_at: '2012-03-25 09:54:09 UTC' })
    @invoice3 = Invoice.create({ customer_id: @customer.id, merchant_id: @merchant3.id,
                                 created_at: '2012-03-25 09:54:09 UTC' })
    @invoice4 = Invoice.create({ customer_id: @customer.id, merchant_id: @merchant4.id,
                                 created_at: '2012-03-25 09:54:09 UTC' })
    @invoice5 = Invoice.create({ customer_id: @customer.id, merchant_id: @merchant1.id,
                                 created_at: '2012-03-27 09:54:09 UTC' })

    @invoice_item1 = InvoiceItem.create({ item_id: @item1.id, invoice_id: @invoice1.id,
                                          quantity: 1, unit_price: @item1.unit_price })
    @invoice_item2 = InvoiceItem.create({ item_id: @item2.id, invoice_id: @invoice2.id,
                                          quantity: 7, unit_price: @item2.unit_price })
    @invoice_item3 = InvoiceItem.create({ item_id: @item3.id, invoice_id: @invoice3.id,
                                          quantity: 1, unit_price: @item3.unit_price })
    @invoice_item4 = InvoiceItem.create({ item_id: @item4.id, invoice_id: @invoice4.id,
                                          quantity: 1, unit_price: @item4.unit_price })
    @invoice_item5 = InvoiceItem.create({ item_id: @item1.id, invoice_id: @invoice1.id,
                                          quantity: 1, unit_price: @item1.unit_price })
